...
당뇨망막병증은 선진국 근로자들의 가장 큰 실명 원인이며, 약 9,300만 명 이상에게 영향을 끼친 것으로 추정되고 있습니다.

정확하고 자동화된 당뇨망막병증 진단의 필요성은 지속적으로 논의되었으며, 이미지 분류와 패턴 인식, 그리고 머신러닝을 이용한 노력 덕분에 많은 발전이 있었습니다. 우리의 목표는 안구 사진을 입력값으로 받아 실제 진단에 사용할 수 있는 새로운 모델을 만들어내는 것입니다.

본 프로젝트의 동기는 두 가지입니다:

* 이미지 분류는 빅데이터 분류와 더불어 지난 수 년간 유망 분야였습니다.

* 환자들이 아래 사진과 같이 안구를 스캔하고, 의사들로부터 진단을 받고, 다음 약속을 잡는 데까지 많은 시간이 낭비됩니다. EyeNet은 실시간으로 이미지를 처리함으로써 진단과 예약을 한 번에 할 수 있도록 할 것입니다.  
...
데이터는 [2015 Kaggle competition](https://www.kaggle.com/c/diabetic-retinopathy-detection)에서 가져왔습니다. 하지만 이는 일반적인 Kaggle 데이터들과는 좀 다릅니다. 대부분의 Kaggle competition에서는 데이터가 이미 정리되어 있어, 데이터 사이언티스트들이 전처리할 부분이 거의 없지만, 이 데이터는 예외입니다. 

모든 사진은 제각기 다른 사람으로부터 다른 카메라를 사용하였으며 다른 크기입니다. [전처리](#전처리) 부분에 관하여, 이 데이터는 노이즈가 아주 많으며 사진들을 모델 학습에 사용 가능한 형태로 만들기 위해서는 여러 단계의 전처리 과정을 거쳐야 합니다.

학습 데이터는 총 35,126장의 사진으로 이루어져 있으며, 전처리 과정 중에 늘어납니다.
...
원래 학습 데이터에서 25,810장의 사진은 망막병증에 걸리지 않은 것으로, 9,316장은 망막병증에 걸린 것으로 분류됩니다.

이런 불균형 때문에, 모델을 학습시키기 위해서는 [전처리](#전처리)로 불균형을 수정하기 위한 단계가 필요합니다.

또한, 안구 사진 간 분산이 너무 높습니다. 첫 두 줄의 사진은 class 0(망막병증 없음)이고, 다음 두 줄은 class 4(증식성 망막병증)입니다. 
...
### EC2로 사진 저장
사진들은 Kaggle CLI를 통해 저장됩니다. EC2에서 이를 실행하면 모두 저장하는 데 약 30분 정도 걸리며, 사진들은 각각의 폴더에 압축이 해제되어 저장될 것입니다. 데이터 용량은 총 35 GB 입니다.

### 사진 자르기 및 크기 변경
모든 사진들은 `256 * 256`으로 조정되어 있습니다. 학습에 오래 걸리더라도, `128 * 128`보다 이 크기의 사진에 더욱 자세하고 많은 정보가 있습니다.

추가적으로, 403장의 사진이 학습셋에서 사라집니다. 이 사진들에는 색상이 없어 Scikit-Image가 크기 변경 과정에서 경고한 사진들입니다. 따라서 완전히 검은색인 사진은 학습 데이터에서 제거됩니다. 

### 사진 회전 및 반전
모든 사진들은 회전 및 반전됩니다. 망막병증이 없는 사진은 반전되고, 망막병증이 있는 사진은 반전 및 90도, 120도, 180도, 그리고 270도 회전됩니다.
...
두 범주를 예측하기 위하여 EyeNet은 3개의 합성곱 층을 사용하며, 각각 32의 깊이를 가지고 있습니다. 맥스 풀링은 마지막에 (2,2) 크기의 합성곱 층 세 개 모두에 적용됩니다.

풀링이 끝나면 데이터는 크기 128의 dense layer로 입력되고, 2개의 소프트맥스 노드를 가진 출력망으로 이동합니다.

![TensorBoard CNN](images/readme/cnn_two_classes_tensorboard.png)

## 결과
EyeNet 분류기는 환자가 망막병증에 걸렸는 지를 판단하기 위해 만들어졌습니다. 현 모델은 다음과 같은 결과를 출력합니다.
...
이 신경망은 어떻게 이렇게 잘 동작하는 것일까요? 분류 범주 간 불균형 외에, 이미지 자르기가 신경망의 성능에 아주 큰 도움을 줍니다. 이미지 내에 검은 부분이 없기 때문에 신경망이 안구 자체만을 처리할 수 있습니다. 

## 다음 단계
1. 새로운 사진들로 재학습시키기 위해 신경망을 프로그래밍합니다. 이는 흔한 과정이며, 모델을 최적화하기 위해 사용됩니다. 저품질 사진들이 분류기를 과도하게 바꾸는 것을 방지하기 위해 사진들이 분류기에 입력되기 전에 검사할 수 있습니다
